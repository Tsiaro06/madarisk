import { apiGet, apiGetPage, apiPost, apiPatch, apiDelete, apiBlob } from './client';
import type {
  AlertListRow,
  AuthTokens,
  CommuneDetail,
...
export const eventsApi = {
  ...
  update: (id: string, body: unknown) => apiPatch<EventListItem>(`/events/${id}`, body),
  updateStatus: (id: string, status: string) =>
    apiPatch(`/events/${id}/status`, { status }),
  remove: (id: string) => apiDelete(`/events/${id}`),
  tracks: (id: string) => apiGet(`/events/${id}/tracks`),
  trackGeoJson: (id: string) => apiGet<FeatureCollection>(`/events/${id}/track-geojson`),
...
export const alertsApi = {
  ...
  update: (id: string, body: unknown) => apiPatch<AlertListRow>(`/alerts/${id}`, body),
...
export const matchingApi = {
  ...
  approve: (id: string, notes?: string) => apiPost(`/matching/${id}/approve`, { notes }),
  reject: (id: string, notes: string) => apiPost(`/matching/${id}/reject`, { notes }),
...
export const usersApi = {
  ...
  update: (id: string, body: unknown) => apiPatch<SanitizedUser>(`/users/${id}`, body),
